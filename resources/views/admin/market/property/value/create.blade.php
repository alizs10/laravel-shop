...
@section('head-tag')
    <title>پنل ادمین | بخش فروش | فرم کالا | مقادیر فرم کالا | ایجاد مقدار جدید</title>
@endsection
@section('breadcrumb')
    <section class="m-2 px-2 py-4 rounded-lg bg-slate-100 dark:bg-slate-800 dark:text-white flex items-center gap-x-2">
        ...
        <a href="{{ route('admin.market.property.value.index', $property->id) }}"
            class="text-xs md:text-base text-purple-800 dark:text-purple-400">مقادیر فرم کالا ({{ $property->name }})</a>
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <span class="text-xs md:text-base">ایجاد مقدار جدید</span>
...
<section class="flex flex-col gap-y-2 p-2 w-full">
    ...
    <div class="flex justify-between items-center">
        <span class="text-sm md:text-lg">ایجاد مقدار جدید</span>
        <a href="{{ route('admin.market.property.value.index', $property->id) }}" class="btn bg-blue-600 text-white">بازگشت</a>
    </div>
    ...
    <form class="w-full" action="{{ route('admin.market.property.value.store', $property->id) }}" method="POST"
        ...
        <section class="w-full grid grid-cols-2 gap-2">
            ...
            <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
                ...
                <input type="text" class="form-input" name="value" id="value" value="{{ old('value') }}">
            </div>
            <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
