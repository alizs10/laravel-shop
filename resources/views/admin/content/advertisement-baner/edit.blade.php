...
@section('head-tag')
    <title>پنل ادمین | بخش محتوی | بنرهای تبلیغاتی | ویرایش بنر جدید</title>
@endsection
@section('breadcrumb')
    <section class="m-2 px-2 py-4 rounded-lg bg-slate-100 dark:bg-slate-800 dark:text-white flex items-center gap-x-2">
        ...
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <span class="text-xs md:text-base">ویرایش بنر جدید</span>
...
@section('content')
    <section class="flex flex-col gap-y-2 p-2 w-full">
        ...
        <div class="flex justify-between items-center">
            <span class="text-sm md:text-lg">ویرایش بنر جدید</span>
        ...
        <form class="w-full" action="{{ route('admin.content.advertisement-baner.update', $baner->id) }}"
            ...
            <section class="w-full grid grid-cols-2 gap-2">
                ...
                <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
                    ...
                    <input type="text" class="form-input" name="url" id="url" value="{{ old('url', $baner->url) }}">
                </div>
                <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
                    <label for="position"
                ...
                <div class="col-span-2 flex flex-col gap-y-1">
                    ...
                    <select name="status" id="status" class="form-select" style="direction: ltr">
                        <option value="1" @if (old('status', $baner->status) == 1) selected @endif>فعال</option>
                        <option value="0" @if (old('status', $baner->status) == 0) selected @endif>غیرفعال</option>
                    </select>
                </div>
