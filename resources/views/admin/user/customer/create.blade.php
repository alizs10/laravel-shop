...
@section('head-tag')
    <title>پنل ادمین | بخش کاربران | مشتریان | ایجاد مشتری</title>
@endsection
@section('breadcrumb')
    <section class="m-2 px-2 py-4 rounded-lg bg-slate-100 dark:bg-slate-800 dark:text-white flex items-center gap-x-2">
        ...
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <span class="text-xs md:text-base">مشتری جدید</span>
...
@section('content')
    ...
    <section class="flex flex-col gap-y-2 p-2 w-full">
        ...
        <div class="flex justify-between items-center">
            <span class="text-sm md:text-lg">مشتری جدید</span>
        ...
        <form class="w-full" action="{{ route('admin.user.customer.store') }}" method="POST"
            ...
            <section class="w-full grid grid-cols-2 gap-2">
                ...
                <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
                    <label for="national_code"
                        class="text-xs {{ $errors->has('national_code') ? 'text-red-600 dark:text-red-400' : '' }}">کد ملی</label>
                    <input type="national_code" class="form-input" name="national_code" id="national_code" value="{{ old('national_code') }}">
                </div>
                <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
                    <label for="password"
                        class="text-xs {{ $errors->has('password') ? 'text-red-600 dark:text-red-400' : '' }}">کلمه
                        عبور</label>
                ...
                <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
                    <label for="password_confirmation"
                        class="text-xs {{ $errors->has('password_confirmation') ? 'text-red-600 dark:text-red-400' : '' }}">تکرار
                        کلمه عبور</label>
