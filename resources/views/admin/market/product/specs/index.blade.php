...
@section('head-tag')
    <title>پنل ادمین | بخش فروش | محصولات | مشخصات کالا</title>
@endsection
@section('breadcrumb')
    <section class="m-2 px-2 py-4 rounded-lg bg-slate-100 dark:bg-slate-800 dark:text-white flex items-center gap-x-2">
        ...
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <span class="text-xs md:text-base">مشخصات کالا ({{ $product->name }})</span>
...
@section('content')
    <section class="flex flex-col gap-y-2 p-2 w-full">
        <div class="flex justify-between items-center">
            <span class="text-sm md:text-lg">مشخصات کالا ({{ $product->name }})</span>
            <a href="{{ route('admin.market.product.spec.create', $product->id) }}"
                class="btn bg-blue-600 text-white">ثبت مشخصات محصول</a>
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="flex items-center gap-x-1">
                                    <a href="{{ route('admin.market.product.spec.edit', $spec->id) }}"
                                        class="btn bg-yellow-500 text-black flex-center gap-1">
                                        <i class="fa-light fa-pen-to-square"></i>
                                        ویرایش
                                    </a>
                                    <form class="m-0"
                                    action="{{ route('admin.market.product.spec.destroy', $spec->id) }}"
                                        method="POST">
                                        @csrf
                                        {{ method_field('delete') }}
