...
@section('head-tag')
    ...
    <title>پنل ادمین | بخش فروش | مشخصات کالا | ویرایش مشخصات</title>
@endsection
@section('breadcrumb')
    <section class="m-2 px-2 py-4 rounded-lg bg-slate-100 dark:bg-slate-800 dark:text-white flex items-center gap-x-2">
        ...
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <a href="{{ route('admin.market.category-spec.manage', $spec->cat_id) }}"
            class="text-xs md:text-base text-purple-800 dark:text-purple-400">مدیریت مشخصات کالا</a>
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <span class="text-xs md:text-base">ویرایش مشخصات کالا</span>
...
@section('content')
    ...
    <section class="flex flex-col gap-y-2 p-2 w-full">
        ...
        <div class="flex justify-between items-center">
            <span class="text-sm md:text-lg">ویرایش مشخصات کالا</span>
            <a href="{{ route('admin.market.category-spec.manage', $spec->cat_id) }}" class="btn bg-blue-600 text-white">بازگشت</a>
        </div>
        ...
        <form class="w-full" action="{{ route('admin.market.category-spec.update', $spec->id) }}"
            ...
            <section class="w-full grid grid-cols-2 gap-2">
                <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
                    ...
                    <input type="text" class="form-input" name="name" id="name" value="{{ old('name', $spec->name) }}">
                </div>
                <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
                    ...
                    <input type="text" class="form-input" name="default_value" id="default_value"
                        value="{{ old('default_value', $spec->default_value) }}">
                </div>
                <div class="col-span-2 flex flex-col gap-y-1">
                    ...
                    <select name="status" id="status" class="form-select" style="direction: ltr">
                        <option value="1" @if (old('status', $spec->status) == 1) selected @endif>فعال</option>
                        <option value="0" @if (old('status', $spec->status) == 0) selected @endif>غیرفعال</option>
                    </select>
                </div>
