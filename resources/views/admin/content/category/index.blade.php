...
@section('content')
    <section class="flex flex-col gap-y-2 p-2 w-full">
        <div class="flex justify-between items-center">
            ...
            @can('create', \App\Models\Content\PostCategory::class)
                <a href="{{ route('admin.content.category.create') }}" class="btn bg-blue-600 text-white">افزودن دسته بندی
                    جدید</a>
            @endcan
        </div>
                ...
                <tbody class="text-xxs md:text-sm">
                    @foreach ($postCategories as $postCategory)
                        <tr>
                            ...
                            <td>
                                <img src="{{ asset('storage\\' . $postCategory->image['indexArray']['small']) }}"
                                    alt="">
                            </td>
                            <td>{{ $postCategory->tags }}</td>
                            @can('update', \App\Models\Content\PostCategory::class)
                                <td>
                            ...
                            <td>
                                <span class="flex items-center gap-x-1">
                                    ...
                                    @endcan
                                    @can('delete', \App\Models\Content\PostCategory::class)
                                        <form class="m-0"
