...
@section('breadcrumb')
    <section class="m-2 px-2 py-4 rounded-lg bg-slate-100 dark:bg-slate-800 dark:text-white flex items-center gap-x-2">
        ...
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <span class="text-xs md:text-base">تخفیف های شگفت انگیز</span>
...
@section('content')
    <section class="flex flex-col gap-y-2 p-2 w-full">
        ...
        <div class="flex justify-between items-center">
            <span class="text-sm md:text-lg">تخفیف های شگفت انگیز</span>
            @can('create', \App\Models\Market\AmazingSale::class)
                <a href="{{ route('admin.market.discount.amazing.create') }}" class="btn bg-blue-600 text-white">ایجاد تخفیف
                    شگفت انگیز
                    جدید</a>
            @endcan
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="flex items-center gap-x-1">
                                    @can('update', \App\Models\Market\AmazingSale::class)
                                    ...
                                    @endcan
                                    @can('delete', \App\Models\Market\AmazingSale::class)
                                        <form class="m-0"
                                            action="{{ route('admin.market.discount.amazing.destroy', $discount->id) }}"
                                            method="POST">
                                            @csrf
                                            {{ method_field('delete') }}
