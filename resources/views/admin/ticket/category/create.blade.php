...
@section('head-tag')
    <title>پنل ادمین | بخش تیکت ها | دسته بندی | ساخت دسته بندی جدید</title>
@endsection
@section('breadcrumb')
    <section class="m-2 px-2 py-4 rounded-lg bg-slate-100 dark:bg-slate-800 dark:text-white flex items-center gap-x-2">
        ...
        <a href="{{ route('admin.ticket.category.index') }}"
            class="text-xs md:text-base text-purple-800 dark:text-purple-400">دسته بندی</a>
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <span class="text-xs md:text-base">ساخت دسته بندی جدید</span>
...
@section('content')
    <section class="flex flex-col gap-y-2 p-2 w-full">
        ...
        <div class="flex justify-between items-center">
            <span class="text-sm md:text-lg">ساخت دسته بندی جدید</span>
            <a href="{{ route('admin.ticket.category.index') }}" class="btn bg-blue-600 text-white">بازگشت</a>
        </div>
        ...
        <form class="w-full" action="{{ route('admin.ticket.category.store') }}" method="POST"
