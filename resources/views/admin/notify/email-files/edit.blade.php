...
@section('head-tag')
    <title>پنل ادمین | بخش اطلاع رسانی | ویرایش فایل</title>
...
@section('breadcrumb')
    <section class="m-2 px-2 py-4 rounded-lg bg-slate-100 dark:bg-slate-800 dark:text-white flex items-center gap-x-2">
        ...
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <span class="text-xs md:text-base">{{ $file->email->subject }}</span>
        <i class="fa-light fa-angles-left text-xs md:text-sm"></i>
        <span class="text-xs md:text-base">ویرایش فایل</span>
...
@section('content')
    <section class="flex flex-col gap-y-2 p-2 w-full">
        ...
        <div class="flex justify-between items-center">
            <span class="text-sm md:text-lg">ویرایش فایل</span>
            <a href="{{ route('admin.notify.email-files.index', $file->email->id) }}"
        ...
        <form class="w-full" action="{{ route('admin.notify.email-files.update', $file->id) }}" method="POST"
            ...
            <section class="w-full grid grid-cols-2 gap-2">
                ...
                <div class="col-span-2 md:col-span-1 flex flex-col gap-y-1">
                    ...
                    <select name="file_save_path" id="file_save_path" class="form-select" style="direction: ltr">
                        ...
                        <option value="0" @if (old('file_save_path', $file->file_save_path) === 1) selected @endif>عمومی</option>
                        <option value="1" @if (old('file_save_path', $file->file_save_path) === 0) selected @endif>خصوصی</option>
                    </select>
                </div>
